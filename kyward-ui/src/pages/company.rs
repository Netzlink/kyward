...
impl Component for CompanyPage {
  ...
  fn update(&mut self, msg: Self::Message) -> ShouldRender {
    match msg {
      ...
      Msg::Get => {
        let req = match Request::get(format!("/api/v1alpha1/company/{0}", self.props.id)).body(Nothing)
        {
          Ok(req) => req,
          Err(err) => {
      ...
      Msg::Delete => {
        let req =
          match Request::delete(format!("/api/v1alpha1/company/{0}", self.props.id)).body(Nothing) {
            Ok(req) => req,
            Err(err) => {
              self.error = Some(anyhow::Error::new(err));
              return true;
